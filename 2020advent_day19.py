...
def possible(rulenum):
    #print(rulenum)
    if rulenum in Possible:
        return Possible[rulenum]
    rule = D[rulenum]
    #print(rule,"OK")
    if rule in ('"a"','"b"'):
        #print([rule[1:-1]],"YES")
        Possible[rulenum] = set([rule[1:-1]])
        return set([rule[1:-1]])
    elif "|" in rule:
        S = set()
        Options = rule.split(" | ")
        for option in Options:
            ...
            if len(Nums) == 1:
                ...
                first = possible(Nums[0])
                second = possible(Nums[1])
                for string in second:
                    S |= set(map(lambda x: x + string, first))
        Possible[rulenum] = S
        return S
    else:
        Nums = rule.split()
...
for message in Messages:
    ...
    if string[0]!="0":
        continue
    elif string.count("0")-string.count("1")<1:
        continue
    elif string[-1]!="1":
        continue
    elif "0" in string[string.index("1"):]:
        continue
    else:
        answer+=1
#237
