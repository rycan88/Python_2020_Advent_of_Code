...
inp = '''striped tan bags contain 2 light silver bags, 1 drab black bag, 2 clear tan bags, 2 mirrored tan bags.
...
L = inp.split("\n")
def getnumbag(string):
    return string[: string.index("bag")-1]


def recursion(string):
    num = string[:string.index(" ")]
    bigbag = string[string.index(" ")+1:]
    if bigbag in Found:
        return int(num)*Found[bigbag]
    if len(D[bigbag]) == 0:
        Found[bigbag] = 1
        return int(num)
    counter = 1
    for bag in D[bigbag]:
        counter+= recursion(bag)
    Found[bigbag] = counter
    counter*=int(num)
    return counter

D = {}
for x in L:
    bigbag = x[:x.index("bag")-1]
    rest = x[x.index("contain")+8:-1]
    if "no other" in x:
        D[bigbag] = []
        continue
    Contains = rest.split(", ")
    Contains = list(map(getnumbag,Contains))
    D[bigbag] = Contains

Found = {}
# -1 since it doesnt include the bag
print(recursion("1 shiny gold")-1)
'''
L = inp.split("\n")
def getbag(string):
    return string[string.index(" ")+1: string.index("bag")-1]

def recursion(bigbag):
    if bigbag == "shiny gold":
        return True
    for bag in D[bigbag]:
        if recursion(bag):
            return True
...
for x in L:
    ...
    if "no other" in x:
        ...
        continue
    Contains = rest.split(", ")
    Contains = list(map(getbag,Contains))
    D[bigbag] = Contains
...
for bigbag in D:
    if recursion(bigbag):
        count+=1
# count-1 since we dont count the shiny gold bag
print(count-1)
'''