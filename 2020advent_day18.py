...
def simplify(line,total):
    ...
    if total == "":
        ...
        for x in line:
            ...
            string+=x
        total = int(string)
        return simplify(line[len(string):],total)
        ...
        if line[0] == "*":
            ...
            total+=int(string)
        return simplify(line[len(string)+1:],total)
def removeplus(line):
    #print(line,"PLUS")
    if "+" not in line:
...
def removebrac(line):
    ...
    for x in line:
        if x == ")":
            ...
            bracstr+=x
            if inbrac == 0:
                string+=str(removebrac(bracstr[1:-1]))
                bracstr = ""
        elif x == "(":
            inbrac += 1
            bracstr+=x
...
total = 0
for homework in L:
    num = removebrac("".join(homework.split(" ")))
    #print(num)
    total+=num
