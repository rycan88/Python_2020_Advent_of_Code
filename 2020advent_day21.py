...
inp = '''jtgktc gnvtb sfb vdlsx sdh przbq cvslrn ccntj hvbfjv rkvs jctgkq cfkv kjz rkkrx fvxql pgfzp vrqkb bqkndvb bmrmhm frdf xgms pvhf cccmn glbktsc phxqzn hqqt bqtvr qxgsq gpddp dtsjv jcpb txjkrbh cnj rlvkp ldxh qlcsjhg fcfvhz vflms zcx jrvgpbht bxtztd bvpq rgf qzkjrtl xfsrjp mzvqjf rtjzgg sbkztg lrc bbkzrl hfrhh hnbvdq snhrpv qknlg khjjdk (contains shellfish, fish)
...
Counter = {}
for food in L:
    allegen = food[food.index("contains")+9:-1].split(", ")
    ingredients = food[:food.index("(")-1].split()
    print(ingredients)
    for a in allegen:
        if a not in D:
            D[a] = set(ingredients)
        else:
            D[a] = D[a] & set(ingredients)
...
print(sum(Counter.values())-answer)
