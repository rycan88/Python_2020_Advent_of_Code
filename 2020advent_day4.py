...
def valid(att,string):
    if att == "byr":
        return 1920<=int(string)<=2002
    elif att == "iyr":
        return 2010<=int(string)<=2020
    elif att == "eyr":
        return 2020<=int(string)<=2030
    elif att == "hgt":
        if "cm" in string:
            return 150<=int(string[:string.index("cm")])<=193
        elif "in" in string:
            return 59<=int(string[:string.index("in")])<=76
        return False
    elif att == "hcl":
        if string[0] == "#":
            for x in string[1:]:
                if x not in ValidHair:
                    return False
            return True
        return False
    elif att == "ecl":
        return (string in EyeColor)
    elif att == "pid":
        return (len(string)==9)
...
for passport in L:
    stats = passport.split()
    Att = {}
    for x in stats:
        Att[x[:x.index(":")]] = x[x.index(":")+1:]
    running = True
    for att in Need:
        if att not in Att:
            running = False
            break
        if valid(att,Att[att]) == False:
            running = False
            break
    if running:
        count+=1
